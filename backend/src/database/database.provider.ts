...
import { dbQueryTotal, dbQueryDuration, dbErrorsTotal, dbConnections, dbIdleConnections } from 'src/metrics/metrics';
    ...
    useFactory: async () => {
      const sequelize = new Sequelize({
        ...
        logging: (sql, timing) => {
          ...
          if (timing) {
            dbQueryDuration.observe({ model, type: queryType }, timing / 1000); // ms -> s
          }
        },
      });
