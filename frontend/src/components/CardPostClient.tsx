...
const CardPost = ({ post }: CardPostProps) => {
	const [author, setAuthor] = useState<{ name: string; profileImgUrl: string } | null>(null)
			...
			<div className="w-11/12 max-w-[600px] bg-white hover:shadow-xl transition-shadow duration-300 flex items-center gap-4 p-5 rounded-3xl border border-gray-200 cursor-pointer">
				<div className="flex flex-col gap-3 flex-1 min-w-0">
					...
					<div className="flex flex-col gap-1 min-w-0">
						<p className="text-lg font-semibold text-gray-900 line-clamp-2 break-words" style={{ height: `${lineHeightTitle * maxLinesTitle}rem`, lineHeight: `${lineHeightTitle}rem` }}>
							{post.title}
						</p>
						<p className="text-sm text-gray-600 break-words line-clamp-3" style={{ height: `${lineHeightDesc * maxLinesDesc}rem`, lineHeight: `${lineHeightDesc}rem` }}>
							{post.description}
						</p>
					</div>
				</div>
