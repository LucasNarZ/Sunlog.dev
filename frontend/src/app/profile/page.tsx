...
const Profile = () => {
	...
	useEffect(() => { if (errorUser) router.push('/sign-in') }, [errorUser, router])
		...
		const handleCreatePostClick = () => router.push('/create-devlog')
		...
		return (
			...
			<button onClick={handleCreatePostClick} className="cursor-pointer mt-4 px-5 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 transition">Create Devlog Entry</button>
			</div>
			</>
			)}
			</div>
			...
			{posts && posts.length > 0 ? (
				...
				{posts.map((post, i) => (
					...
					<div className="mb-1 text-sm text-gray-500">{new Date(post.createdAt).toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric' })}</div>
					<CardPostClient post={post} onDeleteClick={() => confirmDeletePost(post)} />
					</div>
				))}
				</div>
			) : (
			...
			<p className="text-gray-600">Start your devlog by sharing what you’re learning or building.</p>
			<button onClick={handleCreatePostClick} className="cursor-pointer px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 transition">Create your first entry</button>
			</div>
			</div>
			)}
			</div>
			...
			{showModal && postToDelete && (
				...
				<h2 className="text-lg font-semibold mb-3 text-gray-800">Delete this entry?</h2>
