...
import { usersRepositoryToken, postsRepositoryToken, followsRepositoryToken } from 'src/constants'
...
describe('UsersService', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: followsRepositoryToken, useValue: followsRepository },
      ],
    }).compile()
  ...
  describe('getUserByEmail', () => {
    it('should call usersRepository.findOne with correct email', async () => {
      ...
      expect(usersRepository.findOne).toHaveBeenCalledWith({ where: { email: 'a@b.com' } })
    })
  })
  ...
  describe('findUserPublic', () => {
    ...
    it('should throw NotFoundException when user not found', async () => {
      ...
      await expect(service.findUserPublic('1')).rejects.toThrow(NotFoundException)
    })
  })
  ...
  describe('getPostByUser', () => {
    ...
    it('should throw NotFoundException if no posts found', async () => {
      ...
      await expect(service.getPostByUser('1')).rejects.toThrow(NotFoundException)
    })
  })
  ...
  describe('followUser', () => {
    ...
    it('should increment followers and create follow relation', async () => {
      ...
      await expect(service.followUser('1', '2')).resolves.toEqual({ message: 'Followed successfully' })
      expect(usersRepository.increment).toHaveBeenCalledWith('followers', { where: { id: '2' } })
      expect(followsRepository.create).toHaveBeenCalledWith({ followerId: '1', followedId: '2' })
    })
  })
  ...
  describe('unfollowUser', () => {
    ...
    it('should decrement followers and destroy follow relation', async () => {
      ...
      await expect(service.unfollowUser('1', '2')).resolves.toEqual({ message: 'Unfollowed successfully' })
      expect(usersRepository.decrement).toHaveBeenCalledWith('followers', { where: { id: '2' } })
      expect(followsRepository.destroy).toHaveBeenCalledWith({ where: { followerId: '1', followedId: '2' } })
    })
  })
