...
describe('UsersController', () => {
  ...
  describe('findUserBasicInfo', () => {
    it('should call usersService.findUserBasic and return user basic info', async () => {
      const userBasic = { name: 'John', profileImgUrl: 'img.jpg', followers: 10 }
      usersService.findUserBasic!.mockResolvedValue(userBasic)
      expect(await controller.findUserBasicInfo('123')).toEqual(userBasic)
  ...
  describe('findUser', () => {
    it('should throw UnauthorizedException if no user id', async () => {
      ...
      await expect(controller.findUser(req)).rejects.toThrow(UnauthorizedException)
    })

    it('should throw UserNotFoundException if user not found', async () => {
      const req = { user: { userId: '123' } } as AuthRequest
      usersService.findUser!.mockResolvedValue(null)
      await expect(controller.findUser(req)).rejects.toThrow(UserNotFoundException)
  ...
  describe('updateUser', () => {
    it('should call usersService.updateUser with userId and body', async () => {
        const req = { user: { userId: '123' } } as AuthRequest
        const updateDto: updateUserDto = { name: 'New Name', bio: 'New bio', profileImgUrl: 'http://image.url/img.png' }
        usersService.updateUser!.mockResolvedValue('updated')
        expect(await controller.updateUser(req, updateDto)).toEqual('updated')
        expect(usersService.updateUser).toHaveBeenCalledWith('123', updateDto)
    })
  })
  ...
  describe('followUser', () => {
    it('should call usersService.followUser with followerId and followedId', async () => {
      ...
      usersService.followUser!.mockResolvedValue({ message: 'Followed successfully' })
      expect(await controller.followUser(req, dto)).toEqual({ message: 'Followed successfully' })
  ...
  describe('unfollowUser', () => {
    it('should call usersService.unfollowUser with followerId and followedId', async () => {
      const req = { user: { userId: '123' } } as AuthRequest
      const dto: FollowUserDto = { followedId: '456' }
      usersService.unfollowUser!.mockResolvedValue({ message: 'Unfollowed successfully' })
      expect(await controller.unfollowUser(req, dto)).toEqual({ message: 'Unfollowed successfully' })
      expect(usersService.unfollowUser).toHaveBeenCalledWith('123', '456')
    })
  })
  ...
  describe('getFollowUser', () => {
    it('should throw BadRequestException if followedId is not UUID', async () => {
      const req = { user: { userId: '123' } } as AuthRequest
      await expect(controller.getFollowUser(req, 'invalid-uuid')).rejects.toThrow(BadRequestException)
    })
    ...
    it('should call usersService.getFollowUser and return result', async () => {
      ...
      expect(usersService.getFollowUser).toHaveBeenCalledWith('123', validUuid)
    })
  })
})
