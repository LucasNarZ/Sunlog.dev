import { Req, Controller, Get, Post, Body, Patch, UseGuards, Param, BadRequestException } from "@nestjs/common";
...
export class PostsController {
    ...
    @UseGuards(AuthGuard)
    @Get("/like/:likerId")
    async getLikePost(@Req() req:AuthRequest, @Param("likerId") likedId:string){
        const likerId = req?.user?.userId;
        if(!isUUID(likerId)){
